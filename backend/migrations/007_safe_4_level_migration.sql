-- ============================================
-- SAFE 4-LEVEL MIGRATION (No Table Recreation)
-- This version does NOT recreate the trims table
-- ============================================
...
-- STEP 3: Add generation_id column to trims (if not exists)
-- Check if column exists first
SELECT CASE 
    WHEN COUNT(*) > 0 THEN 'Column already exists'
    ELSE 'Adding column'
END as status
FROM pragma_table_info('trims')
WHERE name = 'generation_id';

-- Add column only if it doesn't exist
-- Note: In SQLite, we can't use IF NOT EXISTS for ALTER TABLE
-- We'll handle this in the Python script
...
WHERE generation_id IS NULL OR generation_id = 0;

-- STEP 5: Add index for generation_id
CREATE INDEX IF NOT EXISTS idx_trims_generation ON trims(generation_id);

-- Verification queries
SELECT '=== MIGRATION VERIFICATION ===' as status;
SELECT 'Total trims: ' || COUNT(*) FROM trims;
SELECT 'Trims with generation_id: ' || COUNT(*) FROM trims WHERE generation_id IS NOT NULL;
SELECT 'Orphaned trims: ' || COUNT(*) FROM trims WHERE generation_id IS NULL;
SELECT 'Total generations: ' || COUNT(*) FROM generations;
